...
node extract-images.mjs    # 생성 결과물 → assets/ 축소본 + gallery.json (로컬에서만)
node extract-strings.mjs   # AutoFlowCut 로케일 → strings.json의 ui 블록
node render.mjs            # 10종 × ko/en = 20장 PNG
...
- **`assets/`는 목업에 박히는 크기까지만 줄인 스냅샷이다.** 원본(각 ~800KB)을 커밋하지 않으려는
  것이므로, 더 큰 표시가 필요해지면 `extract-images.mjs`의 `WIDTHS`를 올리고 다시 뽑는다.
