export const ko = {
  ...
  whisk2capcut_what_desc: 'Whisk2CapCut은 Google Labs Whisk AI 이미지 생성과 CapCut 영상 편집을 연결하는 데스크톱 앱입니다(Chrome 확장 프로그램으로도 제공). 일관된 스타일로 여러 AI 이미지를 생성하고, Ken Burns 효과(줌 인/아웃)가 적용된 움직이는 영상으로 바로 변환하세요. 이미지 다운로드만 하는 단순 자동화 도구와 달리, 타임라인이 완성된 CapCut 영상 프로젝트를 바로 만들어줍니다.',
